...
{
	let lines:string[] = (await fs.readFile(join(__dirname, '../documentation', path))).toString().split('\n');
	...
	await fs.writeFile(join(__dirname, '../docs/' + path.replace('.md', '.html')), result);
}

async function main(path='')
{
	base = (await fs.readFile(join(__dirname, './base.html'))).toString();
	const files = (await fs.readdir(join(__dirname, '../documentation', path)));
